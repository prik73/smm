...
const ContactUs = () => {
  ...
  try {
    ...
    const templateParams = {
      ...
      phone: formData.phone, // Sender's Mobile Number
      message: formData.message, // Message
    };

    const response = await emailjs.send(
      "service_r3w263c", // Your Service ID
      "template_tvm2zdk", // Your Template ID
      templateParams,
      "m_isodD-q-2loT5tU" // Your Public Key
    );
            ...
            <Card className="bg-white/80 backdrop-blur-sm shadow-xl border-0 h-full">
              ...
              <CardContent className="p-6 sm:p-8 pt-4 space-y-2 sm:space-y-0">
                {[
                  ...
                  >
                    <div
                      className={`p-2 sm:p-3 bg-${item.color}-100 text-${item.color}-500 rounded-xl`}
                    >
                      {item.icon}
                    </div>
